...
# this SHOULD do the action to kill the nearest enemy(Goombas only for now)
def jump_on_Goomba(positions, enemy_metrics):

    if not enemy_metrics:
        return COMPLEX_MOVEMENT.index(['right', 'B'])  # just makes it to where if there are no enemies at the time mario just runs right
    enemies = sorted(enemy_metrics, key=lambda t: t.get('time_to_collision_frames', float('inf')))
    current_target = enemies[0] # the closest Goomba

    mario_x = positions["mario"]["x"]
    enemy_x = current_target["horizontal_distance"] + mario_x # the Goombas absolute x value
    horizontal_distance = current_target["horizontal_distance"]
    is_coming_towards = current_target["is_coming_towards"]
    time_to_collision = current_target["time_to_collision_frames"]

    action = ['NOOP'] # no action

    # for the horizontal movement
    if horizontal_distance > 20: # if the enemy is to the right
        if is_coming_towards:
            action = ['right', 'B'] # mario should run towards it
    elif horizontal_distance < -20: # enemy is to the  left
        action = ['left', 'B'] if is_coming_towards else ['left']
    else: # the Goomba is close, mario will prepare to jump on it
        action = ['right'] if horizontal_distance >= 0 else ['left']

    # jump timing to kill the Goomba
    kill_jump = False # the jump to kill the Goomba

    if abs(horizontal_distance) < 60 and time_to_collision < 45:
        kill_jump = True
    elif abs(horizontal_distance) < 35: # kill range
        kill_jump = True

    if kill_jump:
        if action == ['right']:
            action = ['right', 'A'] # jump right
        elif action == ['left']:
            action = ['left', 'A'] # jump left
        else:
            action = ['A'] # just a jump

    # some tuning for the jump height for Goombas
    if 'A' in action and abs(horizontal_distance) < 25: # smaller hop for a better kill instead of mario doing huge jump
        pass
    try:
        return COMPLEX_MOVEMENT.index(action)
    except ValueError: # if the action isn't good then it should default to (['right', 'B'])
